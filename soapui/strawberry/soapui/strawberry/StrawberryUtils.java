...
import soapui.strawberry.jgraph.ParameterEntry;
...
//		}
	...
	public static ArrayList<Response> requestWithoutInputs(WsdlOperation wsdlOperation) {
	...
	public static ArrayList<Response> requestsWithInputs (Generator<ParameterEntry> gen, WsdlOperation wsdlOperation) {
		
		ArrayList<String> inputParameterNames = getInputParameterNamesByOperation(wsdlOperation);
		
		ArrayList<Response> responses = new ArrayList<Response>();
		for (ICombinatoricsVector<ParameterEntry> vector : gen) {
			if (canCallOperation(vector, wsdlOperation)) {
...
//							NodeList xmlObjectNodeList = xmlObjectDoc
//									.getElementsByTagName(inputParameterNames.get(i));
////							if (responseNodeList.getLength() == 0)
////								return null;
//							Element xmlObjectElement = (Element) xmlObjectNodeList.item(0);
							///////////////////
					...
					//request.setRequestContent(XmlUtils.serializePretty(reqDoc));
							reqDoc.renameNode(
									copiedNode,//reqDoc.getElementsByTagName(copiedNode.getNodeName()).item(0),
	...
	private static boolean canCallOperation (ICombinatoricsVector<ParameterEntry> vector, WsdlOperation wsdlOperation) {
		for(int i = 0; i < vector.getSize(); i++) {
